...
struct AlloApp : App {
  ...
  // add new GUI to adjust the mass of new star and sun
  Parameter ratio{"/ratio", "", 1.0, "", 0.01, 10.0};
  // add more GUI here
  ControlGUI gui;
  ...
  void reset() {
    ...
    // set the mass of Another fixed star
    float mOther = mSun * ratio;
    mesh.vertex(rv(5));
    // set the color as same as sun
    mesh.color(HSV(0.1f, 0.7f, 0.9f));
    mass.push_back(mOther);
    mesh.texCoord(pow(mOther, 1.0f / 3), 0);
    // assume it is stable
    velocity.push_back(rv(0));
    acceleration.push_back(rv(0));
  }
